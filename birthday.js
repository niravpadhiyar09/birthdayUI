...
function showWish(on){ wishEl.classList.toggle('is-in', on); setPhotos(on); }
...
function armReplay(){
  ...
  requestAnimationFrame(() => {
    ...
    shareBtn.classList.add('is-shown');
  });
}
...
function resetAll(){
  ...
  shareBtn.classList.remove('is-shown', 'is-copied'); shareBtn.hidden = true;
...
/* ============================================================
   PERSONALISATION — a shareable card. The sender names the birthday
   person + signs it; the recipient opens the link and the film greets
...
/* Defaults: a card from Nivid to his wife Nisha. Any param overrides. */
const DEFAULT_TO   = 'Nisha';
const DEFAULT_FROM = 'Nivid';
const DEFAULT_MSG  = 'you are my whole world, today and always';

const card = {
  to:   (params.get('to')   || '').trim() || DEFAULT_TO,
  from: (params.get('from') || '').trim() || DEFAULT_FROM,
  msg:  (params.get('msg')  || '').trim() || DEFAULT_MSG,
};

/* Paint the names into the film. textContent (never innerHTML) so a name can
   never inject markup. Runs before any reveal, so the existing fades/hinges
...
function applyCard(c){
  ...
  if (srLine){
    srLine.textContent =
      `Happy Birthday, ${c.to}.` +
      (c.msg  ? ` ${c.msg}`               : ' Here’s to a wonderful year ahead.') +
      (c.from ? ` With love, ${c.from}.`  : '');
  }
}

/* copies the current link (used by the end-of-film Share button) */
...
/* --- the end-of-film Share button (copies the current card link) --- */
const shareBtn = $('share'), shareLabel = $('shareLabel');
let shareResetTO = 0;
shareBtn.addEventListener('click', async () => {
  const ok = await copyText(location.href);
  shareBtn.classList.add('is-copied');
  shareLabel.textContent = ok ? 'Link copied ✓' : 'Link in address bar';
  clearTimeout(shareResetTO);
  shareResetTO = setTimeout(() => {
...
/* ============================================================
   PHOTOS — cut-outs of the birthday person, blooming among the tree's
   branches in the finale. Drop files in /photos; Vite globs them at
   build time. Reveal is driven by showWish(). ?photos=off disables.
   ============================================================ */
const photoUrls = import.meta.glob('./photos/*.{png,jpg,jpeg,webp}', { eager: true, as: 'url' });
let PHOTOS = Object.keys(photoUrls).sort().map((k) => photoUrls[k]);

/* until real photos are dropped in, numbered placeholders so the layout shows */
function placeholderPhoto(i){
  const hue = ['#f2a9c0', '#f6c26b', '#c58ad6', '#8fc7e6'][i % 4];
  const svg =
    `<svg xmlns='http://www.w3.org/2000/svg' width='300' height='400' viewBox='0 0 300 400'>` +
    `<ellipse cx='150' cy='330' rx='118' ry='95' fill='${hue}'/>` +
    `<circle cx='150' cy='150' r='92' fill='${hue}'/>` +
    `<circle cx='150' cy='150' r='92' fill='rgba(255,255,255,.14)'/>` +
    `<text x='150' y='172' font-family='Georgia' font-size='72' fill='rgba(255,255,255,.9)' text-anchor='middle'>${i + 1}</text>` +
    `</svg>`;
...
function buildPhotos(){
  photosEl.className = 'photos mode-tree';
  photosEl.innerHTML = '';
  if (photosOff) return;
  PHOTOS.forEach((src, i) => {
    const fig = document.createElement('figure');
    fig.className = 'photo photo--' + i;
...
/* called by showWish() on every reveal/hide */
function setPhotos(on){ if (!photosOff) photosEl.classList.toggle('is-in', on); }

/* ?pick=1 → jump straight to the finale (a quick preview of the photos) */
const pickMode = params.get('pick') === '1';
function jumpToFinale(){
  gsap.set(hero, { autoAlpha: 0 });
  gsap.set([flood, field, bloom], { autoAlpha: 0 });
  treeStop();
  drawFinal();            // static finale → showWish(true) → setPhotos(true)
  armReplay();
}
...
applyCard(card);   // paint names first — recipient + reduced-motion both get them
buildPhotos();     // stage the chosen photo layout (reveals with the wish)
...
if (reduceMotion){
  ...
  if (pickMode){
    // preview: jump straight to the finale to see the photos
    requestAnimationFrame(jumpToFinale);
  } else {
    // Everyone opens straight into the film — defaults (or ?to=&from=&msg=).
    enter();
  }
}
